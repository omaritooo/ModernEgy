  ...
  <div id="map-wrap" class="" style="">
    <div v-if="tester">
      <transition name="slide-fade">
        <div
          class="absolute top-0 left-0 w-full h-full p-6 z-555"
          v-if="toggle"
        >
          <div class="relative w-full h-full rounded-lg">
            ...
            <div class="absolute top-4 left-48 right-48">
              <div class="flex">
                ...
                <div>
                  <div
                    class="flex px-2 mx-2 space-x-3 text-white rounded-lg  clicker text-md bg-site-secondary"
                  >
                    <a href="http://thetower.modernegy.com/" target="_blank"
                      >The Tower</a
                    >
                  </div>
                </div>
                    ...
                    <div v-for="dist in data.district" :key="dist.index">
                      <div v-if="dist.title == 'R7 District'">
                        <div v-for="proje in dist.project" :key="proje.index">
                          ...
                          >
                            <a
                              href="http://compound.adgrouptech.com/"
                              target="_blank"
                              >{{ proje.title }}</a
                            >
                          </div>
                      ...
                      <div v-if="dist.title == 'Mu 23'" class="flex flex-row">
                        <div
                          v-for="proje in dist.project"
                          :key="proje.index"
                          class="flex"
                        >
                          <div
                            v-if="name == 'mu23'"
                            class="flex flex-row px-2 mx-2 space-x-3 text-white rounded-lg  clicker text-md bg-site-secondary"
                          >
                            {{ proje.title }}
                          </div>
                        </div>
            ...
            <div class="absolute top-4 right-4">
              <div class="flex space-x-3">
                <button @click="tester = !tester">
                  <font-awesome-icon
                    :icon="['fas', 'expand-arrows-alt']"
                    size="md"
                    class="text-white transition duration-500 ease-in-out transform  text-md hover:-translate-y-1 hover:shadow-xl"
                  />
                </button>
                <button @click="screenToggle">
                  <font-awesome-icon
                    :icon="['fas', 'times']"
                    size="lg"
                    class="text-white transition duration-500 ease-in-out transform  text-md hover:-translate-y-1 hover:shadow-xl"
            ...
            <div class="relative w-full h-full rounded-lg">
              ...
              <div class="absolute top-4 left-48 right-48">
                <div class="flex w-full">
                  ...
                  <div class="container flex flex-row text-lg">
                    <div>
                      <div
                        class="flex px-2 mx-2 space-x-3 text-white rounded-lg  clicker text-md bg-site-secondary"
                      >
                      ...
                      <div v-for="dist in data.district" :key="dist.index">
                        ...
                        <div class="flex flex-row w-full">
                          ...
                          >
                            <div
                              v-if="name == 'mu23'"
                              class="flex flex-row px-2 mx-2 text-white rounded-lg  gap-x-2 clicker text-md bg-site-secondary"
                            >
                              {{ proje.title }}
                            </div>
                          </div>
...
export default {
  ...
  mounted() {
    ...
    axios
      .get("/government.geojson")
      .then((response) => (this.govt = response.data));
    axios
      .get("/downtown.geojson")
      .then((response) => (this.downtown = response.data));
    axios.get("/mu23.geojson").then((response) => (this.mu23 = response.data));
    axios
      .get("/newcapital.geojson")
      .then((response) => (this.nc = response.data));
