      ...
      <div class="flex flex-col justify-center p-4 space-y-2 md:w-1/2 md:p-10">
        ...
        <form v-on:submit.prevent="submitForm()">
          <div class="flex flex-col float-left gap-y-4">
            <label>Email</label>
            <div class="flex flex-col md:block md:gap-x-2 gap-y-2"><input class="p-2 rounded-md" type="email" v-model="data.email" placeholder="Enter your email address...">
            <button @click.prevent="submitForm()" class="w-1/3 px-4 py-2 bg-transparent border-2 border-gray-300 rounded-md ">Click me</button>
            </div>
            </div>
        </form>
      </div>
...
methods: {
  submitForm(){
            axios.post('http://modernegy.adgroup.tech/api/v1/contacts', this.data)
                 .then((res) => {
                   console.log(res)
})
                 .catch((error) => {
